...
class myHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
	...
	def do_GET(self):
		...
		# Get junction locations
			...
			objFile = open('Baze\\Server\\data\\locs.json')
			strData = objFile.read()
			objFile.close()
			self.wfile.write(strData.encode())
		# Get roads
		elif 'getRoads' in self.path:
			objFile = open('Baze\\Server\\data\\roads.json')
			strData = objFile.read()
			objFile.close()
			self.wfile.write(strData.encode())
	...
	def do_POST(self):
		...
		if 'getPath' in self.path:
			if ('Content-Length' in self.headers):
				...
				objLocsFile = open('Baze\\Server\\data\\locs.json')
				objRoadsFile = open('Baze\\Server\\data\\roads.json')
...
print('Ready to serve! Port 8080 /Baze/client/index.html')
httpd.serve_forever();