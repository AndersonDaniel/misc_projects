...
var CONST_NEW_MISSILE_FREQ = 0.01;
...
var CONST_SOURCES = [
		new THREE.Vector3(-34.78903345691955, 2.9576912504549386, -182.02369831595473),
		new THREE.Vector3(-31.704723834475512, 2.994115321960035, -185.18510258294558),
		new THREE.Vector3(-31.028718822704043, 2.8183301528867135, -187.42193534112513),
		new THREE.Vector3(-28.872275111534226, 2.999999999999998, -190.64209577476032),
		new THREE.Vector3(-28.924354765075325, 3.386976753447979, -185.8031499706906),
		new THREE.Vector3(-26.917633782524778, 3.0135695823194175, -190.23950847337872),
		new THREE.Vector3(-28.775153222562192, -1.4079489426575098e-15, -193.65916166649095),
		new THREE.Vector3(-26.114909046856855, 2.6461556502668766, -197.06977945212114),
	];
var time_left;
var current_time = 0;
...
function updateScene() {
	...
	if (Math.random() < CONST_NEW_MISSILE_FREQ) {
		addMissile();
	}
}
...
function loadPolys() {
	$.get('data.json', function(text, status) {
		...
		for (var nIndex = 0; nIndex < data.length; nIndex++) {
			...
			_polygons.push({obj: myMesh, vertices: data[nIndex]});
		}
	});
}
...
function addMissile() {
	var currMissile = {};
		...
		currMissile.attack_model.position.copy(CONST_SOURCES[Math.floor(Math.random() * CONST_SOURCES.length)]);
		console.log(currMissile.attack_model.position);
...
function prepareShootdown(objMissile) {
	...
	target = CalcPositionAtHeight(objMissile.attack_model.position, movement, CONST_H);
...
function CalcPositionAtHeight(curr_pos, movement, height) {
	time_left = (Math.sqrt(Math.pow(movement.vy, 2) + 2 * CONST_G * (curr_pos.y - height)) + movement.vy) / CONST_G;
	pos = new THREE.Vector3(movement.start_position.x + time_left * movement.vx,
							   movement.start_position.y + time_left * movement.vy - (CONST_G / 2) * Math.pow(time_left, 2),
							   movement.start_position.z + time_left * movement.vz);
   return (pos);
}